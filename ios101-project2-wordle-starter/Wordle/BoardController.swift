...
class BoardController: NSObject,
  ...
  func resetBoardWithCurrentSettings() {
    // START YOUR CODE HERE
      let currSetting = SettingsManager.shared.settingsDictionary;
      resetBoard(with: currSetting);
    // END YOUR CODE HERE
  }
  ...
  private func applyIsAlienWordleSettings(with settings: [String: Any]) {
    // START YOUR CODE HERE
      if let alien = settings[kIsAlienWordleKey] as? Int{
          let isAlienWordle = alien;
      }
    // START YOUR CODE HERE
  }
}
